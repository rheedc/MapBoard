...
public class FileUtil {
	...
	//업로드할 폴더가 없으면?
	//자동으로 만들면 된다.
	public static void makeFolder(String fpath) {
	...
	public static String renameTo(String fpath, String foriname) {
		...
		String TempName = foriname;	//현재 이름을 기억해 놓는다.
		//이름과 확장자를 분리한 후 이름을 변경해서 확장자와 합친다.
		
		
		int index = foriname.lastIndexOf(".");	//.을 기준으로 나누기
		String fileName = foriname.substring(0,	 index);	//a
		String extName = foriname.substring(index+1); 	//txt
